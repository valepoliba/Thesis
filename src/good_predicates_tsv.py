...
uninformative = pd.read_csv('datasets/drugbank/uninformative.tsv', sep='\t')
print("Count allpredicates: ", allpredicates.count())
print("Count badpredicates: ", badpredicates.count())
print("Count uninformative: ", uninformative.count())
#print(badpredicates)

df = pd.concat([allpredicates, badpredicates]).drop_duplicates(keep=False)
#print(df)
print("Count: ", df.count())
df.to_csv('datasets/drugbank/good_predicates.tsv', index=False)
