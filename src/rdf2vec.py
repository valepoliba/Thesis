...
class RDF2VecTransformer():
    ...
    def _extract_random_walks(self, graph, instance):
        ...
        walks = graph.extract_random_walks(self.max_path_depth*2, self.walks_per_graph)
    ...
    def fit(self, graph, instances):
        ...
        for i, instance in tqdm.tqdm(enumerate(instances)):
            if self.wl:
                walks = self._extract_wl_walks(graph[i], Vertex(str(instance)))
            else:
                walks = self._extract_random_walks(graph[i],
                                                    Vertex(str(instance)))
        ...
        self.model = Word2Vec(sentences, vector_size=self.vector_size,
                              ...
                              sg=self.sg, epochs=self.max_iter,
