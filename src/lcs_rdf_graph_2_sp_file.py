...
from rdf_graph_utils import root_node, rdf_to_plot
...
class LCS(rdflib.Graph):
    def __init__(self, graph1, graph2, depth, stop_patterns=[], uninformative_triples=[], additional_stop_patterns=[]):
        super().__init__()
        ...
        self.additional_stop_patterns = additional_stop_patterns if len(additional_stop_patterns) > 0 else stop_patterns
    ...
    def explore(self, a, a_T, b, b_T, depth=0):
        ...
        if x:
            ...
            # se x_T è vuoto la risorsa non ha nodi child e se il prefisso "blank_" è nel nome
            # rispetta le condizioni per essere segnata come blank
            if len(x_T) == 0 and "blank_" in x:
                blank = True
        else:
                    ...
                    for (s_a, p_a, o_a) in self.compute_sigma(a, a_T):
                        ...
                        for (s_b, p_b, o_b) in self.compute_sigma(b, b_T):
                            ...
                            if (y_blank or y_uninformative) and z_blank:
                                ...
                                if "blank_" not in y and "blank_" not in z:
    ...
    def compute_sigma(self, node, graph, max_depth=0, depth=0):
        ...
        if "http" in node:
            ...
            sigma = self.__compute_sigma__(query, graph, depth, max_depth)
    ...
    def __make_filter_block__(self, graph=None):
        ...
        if len(self.stop_patterns) == 0:
            return ""
        stops = list(self.stop_patterns) if self.actual_depth < 2 else list(self.additional_stop_patterns)
    ...
    def __compute_sigma__(self, query, graph, depth, max_depth):
...
def blank_node():
    # return rdflib.BNode()
    return rdflib.BNode("blank_%s" % str(md5(str(time()).encode()).hexdigest()))
